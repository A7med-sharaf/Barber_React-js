import "./Text.css"

const TextMain = () => {
  return (
    <div className="TextMain">
      <h1 data-aos="fade-right">Best Haircut and Shave in New York</h1>
      <div data-aos="fade-left" className="text-book">
        <p>Article evident arrived express highest men did boy. Mistress 
          sensible entirely am so. Quick can manor smart money hopes worth too.
           Comfort produce husband boy her had hearing. Law others theirs passed but wishes.
            You day real less till dear read. Considered use dispatched melancholy sympathize 
            discretion led. <br/>
            Oh feel if up to till like. He an thing rapid these after going drawn or.
             Timed she his law the spoil round defer. In surprise concerns informed betrayed he 
             learning is ye.</p>
        <div data-aos="fade-left" className="bootBoreder">BOOK AN APPOINTMENT</div>
      </div>
    </div>
  )
}

export default TextMain